    ...
    <div className={`relative mt-6 mb-4 ${width}`} ref={dropdownRef}>
      ...
      <div className="relative">
        ...
        >
          ...
          <ChevronDown
            className={`w-3 h-3 ml-2 text-gray-400 transition-transform ${
              isOpen ? "rotate-180" : ""
            }`}
          />
        </button>
            ...
            >
              {options.map((option) => (
                <li
                  ...
                  className={`px-3 py-3 text-xs cursor-pointer hover:bg-gray-100 ${
      ...
      {touched && required && !selectedOption && (
        <p className="mt-1 text-[10px] text-red-400">Please select an option</p>
      )}
    </div>
  );
};
